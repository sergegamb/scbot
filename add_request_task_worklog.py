...
from sc.interfaces import RequestTaskTimeEntryInterface
...
async def get_worklog_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Receive request task worklog {update.message.text} from {owner}")
    ...
    answer = RequestTaskTimeEntryInterface.add(request_id, task_id, owner, start_time, end_time, description)
...
add_request_task_worklog_handler = ConversationHandler(
    entry_points=[CallbackQueryHandler(add_worklog, "add_request_task_worklog")],
